...
class RelationController extends Controller
{
   public function index(Request $request)
   {
      ...
      try {
         $data = Relationship::join('student_relations', 'relationships.id', '=', 'student_relations.relation_id')
            ->join('students', 'student_relations.student_id', '=', 'students.id')
         ...
         // dd($data);
         return view('components.relation.data-relation')->with('data', $data);
      } catch (Exception $err) {
        return dd($err);
      }
   }
   ...
   public function getById($id)
   {
      ...
      try {
         ...
         {
            $dataRelationship = Relationship::join('student_relations', 'relationships.id', '=', 'student_relations.relation_id')
               ->join('students', 'student_relations.student_id', '=', 'students.id')
               ->where('relationships.id', $id) 
               ->select('relationships.*', 'students.name as student_name')
               ->first();

            $roles = Roles::where('id','=',$dataRelationship->role_id)->value('name');
         ...
         elseif(session('role') == 'student')
         {
            ...
            $dataRelationship = Relationship::join('student_relation', 'relationships.id', '=', 'student_relation.relation_id')
               ->join('students', 'student_relation.student_id', '=', 'students.id')
               ->join('users', 'users.id', '=', 'relationships.user_id')
               ->where('student_relation.student_id', $idStudent) 
   ...
   //       } 
   //       else if(isset($request->subject_taught) && isset($request->subject_taught)){
   //          $credentials_teacher_grade = [
   //             'teacher_id' => $getIdLastTeacher,
   //             'grade_id' => $request->grade_taught,
   //             'created_at' => now(),
